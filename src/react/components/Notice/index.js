...
const Notice = ( props ) => {
	...
	const containerClasses = classNames( className, 'dlx-app-admin__notice', {
		'dlx-app-admin__notice--has-icon': hasIcon(),
		[ `dlx-app-admin__notice-type--${ status }` ]: true,
		[ `dlx-app-admin__notice-appearance--inline` ]: inline,
		[ `dlx-app-admin__notice-appearance--block` ]: ! inline,
	} );
	...
	return (
		<div className={ containerClasses }>
			<WPNotice isDismissible={ false } spokenMessage={ message } actions={ hasToTop ? actions : [] } { ...props }>
				{ hasIcon() &&
					<div className="dlx-app-admin__notice-icon">{ getIcon( icon ) }</div>
				}
				<div className="dlx-app-admin__notice-message"><>{ message } { children } </></div>
			</WPNotice>
		</div>
	);
};
